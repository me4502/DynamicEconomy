...
public class DynamicEconomyCommandExecutor implements org.bukkit.command.CommandExecutor {
                    ...
                    if (DynamicEconomy.location_restrict) {
                        if (y <= DynamicEconomy.maximum_y & y >= DynamicEconomy.minimum_y) {
                            ...
                            color.sendColouredMessage(player, DynamicEconomy.prefix
                                    + "&2You are too deep underground to access the economy!.");
                            Utility.writeToLog(player.getName() + " called /sell but was too deep underground.");
                        } else if (y >= DynamicEconomy.maximum_y) {
                            color.sendColouredMessage(player, DynamicEconomy.prefix
                                    + "&2You are too high up to access the economy!.");
                            Utility.writeToLog(player.getName() + " called /sell but was too high up.");
                        }
                    } else {
                        trans.sell(player, args);
        ...
        if (cmd.getName().equalsIgnoreCase("dynamiceconomyreloadconfig")) {
            ...
            if (player != null) {
                ...
                if (isInWorld(player) == false) {
                    color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.wrongWorld);
                    Utility.writeToLog(player.getName()
                            + " tried to call /dynamiceconomyreloadconfig, but was in the wrong world.");
                }
                name = player.getName();
            }
            ...
            if (hasPerm) {
                ...
                color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.noPermission);
                Utility.writeToLog(player.getName()
                        + " called /dynamiceconomyreloadconfig but did not have permission.");
            }
            ...
            if (permission.has(player, "dynamiceconomy.update")) {
                if (isInWorld(player)) {
                    if (DynamicEconomy.enableUpdateChecker) {
                        ...
                        if (!isLatest) {
                            color.sendColouredMessage(player, DynamicEconomy.prefix
                                    + "&2New Version of DynamicEconomy Available!");
                        } else {
                            color.sendColouredMessage(player, DynamicEconomy.prefix
                                    + "&2You have the latest version of DynamicEconomy!");
                        }
                    }
                } else {
                    color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.wrongWorld);
        ...
        if (cmd.getName().equalsIgnoreCase("dyneconupdate")) {
            ...
            if (hasPerm) {
                if (isInWorld(player)) {
                    if (DynamicEconomy.enableUpdateChecker) {
                        Utility.writeToLog(player.getName()
                                + " called /dyneconupdate and downloaded the latest version of DynamicEconomy");
                        updater.forceDownload("http://dl.dropbox.com/u/30676341/DynamicEconomy.jar", "DynamicEconomy",
                                player);
                    }
                } else {
                    color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.wrongWorld);
        ...
        if (cmd.getName().equalsIgnoreCase("removefromgroup")) {
            if (permission.has(player, "dynamiceconomy.groups")) {
                if (isInWorld(player)) {
                    ...
                    color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.wrongWorld);
                    Utility.writeToLog(player.getName()
                            + " tried to call /removefromgroup, but was in the wrong world.");
                }
            } else {
                color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.noPermission);
        ...
        if (cmd.getName().equalsIgnoreCase("removegroupfromuser")) {
            if (permission.has(player, "dynamiceconomy.groups")) {
                if (isInWorld(player)) {
                    ...
                    color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.wrongWorld);
                    Utility.writeToLog(player.getName()
                            + " tried to call /removegroupfromuser, but was in the wrong world.");
                }
            } else {
                color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.noPermission);
        ...
        if (cmd.getName().equalsIgnoreCase("removeregiongroup")) {
            if (permission.has(player, "dynamiceconomy.shopregion")) {
                if (isInWorld(player)) {
                    ...
                    color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.wrongWorld);
                    Utility.writeToLog(player.getName()
                            + " tried to call /removeregiongroup, but was in the wrong world.");
                }
            } else {
                color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.noPermission);
        ...
        if (cmd.getName().equalsIgnoreCase("unbanregionitem")) {
            if (permission.has(player, "dynamiceconomy.shopregion")) {
                if (isInWorld(player)) {
                    ...
                    color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.wrongWorld);
                    Utility.writeToLog(player.getName()
                            + " tried to call /unbanregionitem, but was in the wrong world.");
                }
            } else {
                color.sendColouredMessage(player, DynamicEconomy.prefix + Messages.noPermission);
    ...
    public DynamicEconomyCommandExecutor(DynamicEconomy plugin, PluginDescriptionFile desc, FileConfiguration conf,
            File cf) {
        this.plugin = plugin;
