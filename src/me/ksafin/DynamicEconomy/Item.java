...
public class Item {
    ...
    // public static boolean DynamicEconomy.usestock =
    // DynamicEconomy.DynamicEconomy.usestock;
    // public static boolean DynamicEconomy.useboundaries =
    // DynamicEconomy.DynamicEconomy.useboundaries;
    ...
    public static void addAlias(Player player, String[] args) {
        if (args.length != 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/addAlias [Alias] [Item]");
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&f" + alias
                    + "&2 has been added as an alias for &f" + item);
    ...
    public static void removeAlias(Player player, String[] args) {
        if (args.length != 1) {
            ...
            if (DynamicEconomy.aliasConfig.contains(alias) == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&f" + alias
                        + "&2 does not exist as an alias.");
                Utility.writeToLog(player.getName() + " tried to remove " + alias
                        + " as an alias, but it doesn't exist.");
                return;
            }
    ...
    private static String getGroup(String item) {
        ...
        while (i.hasNext()) {
            ...
            for (int x = 0; x < groupItems.size(); x++) {
                ...
                if (curGroupItem.equalsIgnoreCase(item)) return group;
            }
        }
    ...
    public static void canISell(Player player, String[] args) {
        if (args.length != 1) {
            ...
            if (item.equals("")) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Item &f" + args[0] + " &2doesn't exist.");
                Utility.writeToLog(player.getName() + " called /canisell for item " + args[0]
                        + ", which doesn't exist.");
            } else {
    ...
    public static void addItemGroup(Player player, String[] args) {
        if (args.length != 1) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/creategroup [ItemGroupName]");
            ...
            if (DynamicEconomy.groupsConfig.contains(args[0])) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Item Group &f" + args[0]
                        + " &2already exists.");
                Utility.writeToLog(player.getName() + " attempted to create new Item Group " + args[0]
    ...
    public static void addItemToGroup(Player player, String[] args) {
        if (args.length != 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/addtogroup [ItemGroup] [Item]");
            ...
            if (item.equals("")) {
                ...
                Utility.writeToLog(player.getName() + " attempted to add " + args[1] + " to group " + args[0]
            ...
            if (DynamicEconomy.groupsConfig.contains(args[0]) == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Item Group &f" + args[0]
                        + " &2doesn't exist.");
                Utility.writeToLog(player.getName() + " attempted to add an item to Item Group " + args[0]
                        + " but this group doesn't exist.");
                return;
            }
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Item &f" + item + " &2added to group "
                    + args[0]);
    ...
    public static void removeItemFromGroup(Player player, String[] args) {
        if (args.length != 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/removefromgroup [ItemGroup] [Item]");
            ...
            if (item.equals("")) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Item &f" + args[1] + "&2 doesn't exist.");
                Utility.writeToLog(player.getName() + " attempted to remove " + args[1] + " from group " + args[0]
                        + ", but this item doesn't exist.");
                return;
            }
            ...
            if (DynamicEconomy.groupsConfig.contains(args[0]) == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Item Group &f" + args[0]
                        + " &2doesn't exist.");
                Utility.writeToLog(player.getName() + " attempted to remove an item from Item Group " + args[0]
                        + " but this group doesn't exist.");
                return;
            }
            ...
            if (curGroup.contains(item) == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Item Group &f" + args[0]
                        + " &2doesn't contain &f" + item);
                Utility.writeToLog(player.getName() + " attempted to remove an item from Item Group " + args[0]
                        + " but this group doesn't contain " + item);
                return;
            }
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Item &f" + item + " &2removed from group "
                    + args[0]);
    ...
    public static void addGroupToUser(Player player, String[] args) {
        if (args.length != 3) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/addgrouptouser [User] [ItemGroupName] [Purchase|Sale]");
            ...
            if (DynamicEconomy.usersConfig.contains(user) == false) {
                ...
                Utility.writeToLog(player.getName() + " tried to add a group for user " + user
            ...
            if (type.equalsIgnoreCase("PURCHASE")) {
                ...
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The group type &f" + type
                        + "&2 doesn't exist.");
                Utility.writeToLog(player.getName() + " tried to add a group of type " + type
                        + " but this type doesn't exist.");
                return;
            }
            ...
            if (DynamicEconomy.groupsConfig.contains(group) == false && group.equals("*") == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The group &f" + group
                        + "&2 doesn't exist.");
                Utility.writeToLog(player.getName() + " tried to add group  " + group + " for user " + user
                        + ", but this group doesn't exist.");
                return;
            }

            if (curGroups.contains(group) || curGroups.contains("*")) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The user &f" + user
                        + "&2 already has access to group &f" + group);
                Utility.writeToLog(player.getName() + " tried to add a group for user " + user
                        + " but this user already has access to it.");
                return;
            }
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The group &f" + group
                    + "&2 has been added for user &f" + user);
    ...
    public static void removeGroupFromUser(Player player, String[] args) {
        if (args.length != 3) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/removegroupfromuser [User] [ItemGroupName] [Purchase|Sale]");
            ...
            if (DynamicEconomy.usersConfig.contains(user) == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The user &f" + user + "&2 doesn't exist.");
                Utility.writeToLog(player.getName() + " tried to remove a group from user " + user
                        + " but this user doesn't exist.");
                return;
            }
            ...
            if (type.equalsIgnoreCase("PURCHASE")) {
                ...
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The group type &f" + type
                        + "&2 doesn't exist.");
                Utility.writeToLog(player.getName() + " tried to remove a group of type " + type
                        + " but this type doesn't exist.");
                return;
            }
            ...
            if (DynamicEconomy.groupsConfig.contains(group) == false && group.equals("*") == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The group &f" + group
                        + "&2 doesn't exist.");
                Utility.writeToLog(player.getName() + " tried to remove group  " + group + " from user " + user
                        + ", but this group doesn't exist.");
                return;
            }
            ...
            if (curGroups.contains("*") && group.equals("*") == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The user &f" + user
                        + "&2 has access to all groups via group &f'*'");
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2You must remove group &f'*'");
                Utility.writeToLog(player.getName() + " tried to remove group from user " + user
                        + " but this user has access to all groups via '*'");
                return;
            }

            if (curGroups.contains(group) == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2User &f" + user
                        + "&2 doesn't have access to group &f" + group);
                Utility.writeToLog(player.getName() + " tried to remove group from user " + user
                        + " but this user doesn't have access to this group.");
                return;
            }
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The group &f" + group
                    + "&2 has been removed from user &f" + user);
    ...
    public static void viewGroup(Player player, String[] args) {
        ...
        if (args.length != 1 && args.length != 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/viewgroup [ItemGroupName] (Page)");
        ...
        color.sendColouredMessage(player, "&2----------------&fPage &f" + (page + 1) + "/" + numPages
                + "&2----------------");

    }

    public static boolean isItemInRegionGroup(List<String> allowedItemGroups, String item) {
        if (allowedItemGroups.contains("*")) return true;

        for (int x = 0; x < allowedItemGroups.size(); x++) {
            if (DynamicEconomy.groupsConfig.getStringList(allowedItemGroups.get(x)).contains(item)) return true;
        }
    ...
    public static void removeItemGroup(Player player, String[] args) {
        if (args.length != 1) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/removegroup [ItemGroupName]");
            ...
            if (DynamicEconomy.groupsConfig.contains(args[0]) == false) {
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Item Group &f" + args[0]
                        + " &2doesn't exist.");
                Utility.writeToLog(player.getName() + " attempted to remove Item Group " + args[0]
    ...
    public boolean getPrice(Player player, String[] args) {
        ...
        if (args.length < 1 || args.length > 3) {
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Price of &f" + item + "&2 is &f"
                    + DynamicEconomy.currencySymbol + priceStr);
        ...
        if (type.equalsIgnoreCase("sale") || type.equalsIgnoreCase("sell")) {
            ...
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The type &f" + type
                    + "&2 does not exist. Use either buy or sell.");
        ...
        if (type.equalsIgnoreCase("sale") || type.equalsIgnoreCase("sell")) {
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Selling &f" + amt + "&2 of &f" + item
                    + "&2 + " + DynamicEconomy.currencySymbol + tax + " tax &fyields &2" + totalStr);
        } else if (type.equalsIgnoreCase("purchase") || type.equalsIgnoreCase("buy")) {
            color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Buying &f" + amt + "&2 of &f" + item + "&2 + "
                    + DynamicEconomy.currencySymbol + tax + " tax &fcosts &2" + totalStr);
        }
        color.sendColouredMessage(player, DynamicEconomy.prefix + "&2-----------------------------------");
        if (args.length == 2) {
    ...
    public void setPrice(Player player, String[] args) {
        ...
        if (args.length != 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/setprice [Item] [Price]");
            ...
            if (item.equals("")) {
                ...
                if (withinbounds) {
                    ...
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Price of " + item + " set to &f"
                            + DynamicEconomy.currencySymbol + price);
                    saveItemFile();
                    ...
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2MIN: &f"
                            + DynamicEconomy.currencySymbol + floor + " &2MAX: &f" + DynamicEconomy.currencySymbol
                            + ceiling);
        ...
        if (args.length != 2) {
            ...
            if (item.equals("")) {
                ...
                if (withinbounds == false) {
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The price of " + item
                            + " is lower than desired floor.");
                    color.sendColouredMessage(player, DynamicEconomy.prefix
                            + "&2You must increase price above desired floor, or set lower floor.");
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2DESIRED FLOOR: &f"
                            + DynamicEconomy.currencySymbol + floor + " PRICE: &f" + DynamicEconomy.currencySymbol
                            + price);
                    Utility.writeToLog(stringPlay + " attempted to set floor of " + item + " to " + floor
                            + ", but the price is lower than the desired floor.");
                } else {
                    itemConfig.set(request, floor);
                    saveItemFile();
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Price Floor of &f" + item
                            + " set to&f " + DynamicEconomy.currencySymbol + floor);
    ...
    public void setCeiling(Player player, String[] args) {
        ...
        if (args.length != 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/setceiling [Item] [FloorPrice]");
            ...
            if (item.equals("")) {
                ...
                if (withinbounds == false) {
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2The price of " + item
                            + " is higher than desired ceiling.");
                    color.sendColouredMessage(player, DynamicEconomy.prefix
                            + "&2You must decrease price to below desired ceiling, or set higher ceiling.");
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2DESIRED CEILING: &f"
                            + DynamicEconomy.currencySymbol + ceiling + " PRICE: &f" + DynamicEconomy.currencySymbol
                            + price);
                    Utility.writeToLog(stringPlay + " attempted to set ceiling of " + item + " to " + ceiling
                            + ", but the price is higher than the desired ceiling.");
                } else {
                    itemConfig.set(request, ceiling);
                    saveItemFile();
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Price Ceiling of &f" + item
                            + " set to &f" + DynamicEconomy.currencySymbol + ceiling);
    ...
    public void getFloor(Player player, String[] args) {
        ...
        if (args.length != 1) {
            ...
            if (item.equals("")) {
                ...
                decFormat.applyPattern("#.##");
                floor = Double.valueOf(decFormat.format(floor));
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Price Floor of &f" + item + " &2is &f"
                        + DynamicEconomy.currencySymbol + floor);
    ...
    public void getCeiling(Player player, String[] args) {
        ...
        if (args.length != 1) {
            ...
            if (item.equals("")) {
                ...
                decFormat.applyPattern("#.##");
                ceiling = Double.valueOf(decFormat.format(ceiling));
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Price Ceiling of &f" + item + "&2 is &f"
                        + DynamicEconomy.currencySymbol + ceiling);
    ...
    public void getVelocity(Player player, String[] args) {
        ...
        if (args.length != 1) {
            ...
            if (item.equals("")) {
                ...
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Velocity of &f" + item + " &2is &f"
                        + velocity);
    ...
    public void setVelocity(Player player, String[] args) {
        ...
        if (args.length != 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/setvelocity [Item] [Velocity]");
            ...
            if (item.equals("")) {
                ...
                saveItemFile();
                color.sendColouredMessage(player, DynamicEconomy.prefix + "&2Velocity of &f" + item + " &2set to &f"
                        + velocity);
    ...
    public static void banItem(Player player, String[] args) {
        ...
        if (args.length != 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/banitem [Item] [sale|purchase]");
            ...
            if (item.equals("")) {
                ...
                if (args[1].equalsIgnoreCase("sale") || args[1].equalsIgnoreCase("purchase")) {
                    ...
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&f" + args[0] + "&2 banned from &f"
                            + args[1] + "&2 succesfully.");
                    ...
                    color.sendColouredMessage(player, DynamicEconomy.prefix + "&f" + args[1]
                            + "&2 is not a valid type. Only use &fsale &2or &fpurchase");
    ...
    public static void unbanItem(Player player, String[] args) {
        ...
        if (args.length != 2) {
            color.sendColouredMessage(player, DynamicEconomy.prefix
                    + "&2Wrong Command Usage. &f/unbanitem [Item] [sale|purchase]");
            ...
            if (item.equals("")) {
                ...
                if (args[1].equalsIgnoreCase("sale") || args[1].equalsIgnoreCase("purchase")) {
                    ...
                    if (contains) {
                        ...
                        color.sendColouredMessage(player, DynamicEconomy.prefix + "&f" + args[0]
                                + "&2 unbanned from &f" + args[1] + "&2 succesfully.");
                        ...
                        Utility.writeToLog(player.getName() + " called /unbanitem on " + args[0]
                                + " which is not banned.");
                    }
    ...
    public static void getDurCommand(Player player, String[] args) {
        ...
        if (args.length == 0) {
            ...
            if (itemID >= 256 && itemID <= 258 || itemID >= 267 && itemID <= 279 || itemID >= 298 && itemID <= 317
                ...
                color.sendColouredMessage(player, "&2The durability of your item is at &f" + percentDur
                        + "%.&2 You have &f" + usesLeft + "&2 uses left out of a possible &f" + maxDur);
            ...
            if (args[0].equalsIgnoreCase("armor")) {
                ...
                if (amountHelmet == 0 && amountChestplate == 0 && amountLeggings == 0 && amountBoots == 0) {
                    ...
                    Utility.writeToLog(stringPlay
                            + " attempted to call /getdurability armor, but has no armor equipped");
                } else {
                    for (int x = 0; x < armor.length; x++) {
                        ...
                        if (amount == 0) {
                            if (x == 3) {
                                ...
                                Utility.writeToLog(stringPlay
                                        + " called /getdurability armor without having a helmet equipped");
                            } else if (x == 2) {
                                color.sendColouredMessage(player, Messages.noChestplateEquipped);
                                Utility.writeToLog(stringPlay
                                        + " called /getdurability armor without having a chestplate equipped");
                            } else if (x == 1) {
                                color.sendColouredMessage(player, Messages.noLeggingsEquipped);
                                Utility.writeToLog(stringPlay
                                        + " called /getdurability armor without having leggings equipped");
                            } else {
                                color.sendColouredMessage(player, Messages.noBootsEquipped);
                                Utility.writeToLog(stringPlay
                                        + " called /getdurability armor without having boots equipped");
                            }
                        } else {
                            percentDur = Double.valueOf(decFormat.format(percentDur));
                            color.sendColouredMessage(player, "&2The durability of &f" + itemName + "&2 is at &f"
                                    + percentDur + "%.&2 You have &f" + usesLeft + "&2 uses left out of a possible &f"
                                    + maxDur);
                            Utility.writeToLog(stringPlay
                                    + " called /getdurability and found the durability of their '" + itemName
                                    + "' to be " + percentDur + "%");
                        }
                    }
                }
                ...
                if (amount == 1) {
                    ...
                    percentDur = 100 - percentDur;
                    percentDur = Double.valueOf(decFormat.format(percentDur));
                    color.sendColouredMessage(player, "&2The durability of &f" + itemName + "&2 is at &f" + percentDur
                            + "%.&2 You have &f" + usesLeft + "&2 uses left out of a possible &f" + maxDur);
                    Utility.writeToLog(stringPlay + " called /getdurability and found the durability of their '"
                            + itemName + "' to be " + percentDur + "%");
                } else {
                    color.sendColouredMessage(player, Messages.noHelmetEquipped);
                    Utility.writeToLog(stringPlay
                            + " attempted to call /getdurability helmet, but did not have a helmet equipped");
                }
                ...
                if (amount == 1) {
                    ...
                    color.sendColouredMessage(player, Messages.noChestplateEquipped);
                    Utility.writeToLog(stringPlay
                            + " attempted to call /getdurability chestplate, but did not have a chestplate equipped");
                }
                ...
                if (amount == 1) {
                    ...
                    color.sendColouredMessage(player, Messages.noLeggingsEquipped);
                    Utility.writeToLog(stringPlay
                            + " attempted to call /getdurability leggings, but did not have leggings equipped");
                }
                ...
                if (amount == 1) {
                    ...
                    color.sendColouredMessage(player, Messages.noBootsEquipped);
                    Utility.writeToLog(stringPlay
                            + " attempted to call /getdurability boots, but did not have boots equipped");
                }
            } else {
                color.sendColouredMessage(player, DynamicEconomy.prefix
                        + "&2Wrong Command Usage. &f/getdurability (helmet/chestplate/leggings/boots/armor)");
