...
public class ExtrasEvents extends ExtrasEventMethods {
    ...
    public boolean didRotate(PlayerMoveEvent evt) {
        ...
        float toYaw = to.getYaw(), fromYaw = from.getYaw(), toPitch = to.getPitch(), fromPitch = from.getPitch();

        if (toYaw != fromYaw || toPitch != fromPitch) return true;
        return false;
    }

    @Override
    public boolean didMove(PlayerMoveEvent evt) {
        if (evt.getTo().getX() != evt.getFrom().getX() || evt.getTo().getY() != evt.getFrom().getY()
                || evt.getTo().getZ() != evt.getFrom().getZ()) return true;
        return false;
    }
